...
def run(seed,n,sigma,lamda,lr,device,m,mode):
    ...
    def validation(epoch,path_checkpoint,initial_weight,last_loss,lbeta_0,lbeta_1,lbeta_01,train_loss):
        ...
        if epoch == 1000:
            ...
            #     size = 0
            #print('Saving..')
            sigma_esti_0 = np.sqrt(train_loss / lbeta_0/ 0.8/n)
            sigma_esti_1 = np.sqrt(train_loss / lbeta_1/ 0.8/n)
            sigma_esti_0_val = np.sqrt(last_loss / lbeta_0/ 0.8/n)
            sigma_esti_1_val = np.sqrt(last_loss / lbeta_1/ 0.8/n)
