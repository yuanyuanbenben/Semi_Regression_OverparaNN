    ...
    if True:
        ...
        for k in range(2,10):
            ...
            try:
                ...
                I = np.eye(Mat_size,Mat_size)
                coef = np.matmul(np.linalg.inv(np.matmul(np.transpose(Mat),Mat)+1e-10*I),np.matmul(np.transpose(Mat),Y))
                beta_hat = coef[0:2,:]
                ...
                Mat_validation = np.concatenate((X_validation,concat_phi(Phi1_validation,Phi2_validation,Phi3_validation,Phi4_validation,Phi5_validation,Phi6_validation)),axis = 1)
